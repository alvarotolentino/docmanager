...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
  ...
  CREATE USER $DOC_USER WITH PASSWORD '$DOC_PWD';
  CREATE DATABASE $METADATA_DB;
  GRANT ALL PRIVILEGES ON DATABASE $METADATA_DB TO $DOC_USER;
  CREATE DATABASE $DATA_DB;
  GRANT ALL PRIVILEGES ON DATABASE $DATA_DB TO $DOC_USER;

  \connect $METADATA_DB $DOC_USER
  BEGIN;
